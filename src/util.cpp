...
const char * const BITCOIN_CONF_FILENAME = "lifetioncoin.conf";
const char * const BITCOIN_PID_FILENAME = "lifetioncoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "lifetioncoin" is a composite category enabling all LifetionCoin-related debug output
            if(ptrCategory->count(string("lifetioncoin"))) {
...
#else
    const char* pszModule = "lifetioncoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LifetionCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LifetionCoinCore
    // Mac: ~/Library/Application Support/LifetionCoinCore
    // Unix: ~/.lifetioncoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LifetionCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LifetionCoinCore";
#else
    // Unix
    return pathRet / ".lifetioncoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lifetioncoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lifetioncoin.conf
