...
*Setup instructions for a Gitian build of LifetionCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LifetionCoin
...
- [Building LifetionCoin Core](#building-lifetioncoin-core)
...
Clone the git repositories for LifetionCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cointobanks/lifetioncoin
```
...
in the LifetionCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build LifetionCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the LifetionCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lifetioncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cointobanks/lifetioncoin
...
For example:
```bash
URL=https://github.com/crowning-/lifetioncoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lifetioncoin=${COMMIT} --url lifetioncoin=${URL} ../lifetioncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lifetioncoin=${COMMIT} --url lifetioncoin=${URL} ../lifetioncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lifetioncoin=${COMMIT} --url lifetioncoin=${URL} ../lifetioncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lifetioncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lifetioncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lifetioncoin.git
SIGPATH=/some/root/path/lifetioncoin-detached-sigs.git

./bin/gbuild --url lifetioncoin=${BTCPATH},signature=${SIGPATH} ../lifetioncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lifetioncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lifetioncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lifetioncoin-osx-build.assert
```
...
[lifetioncoin/gitian.sigs](https://github.com/cointobanks/gitian.sigs/) repository, or if that's not possible to create a pull
request.
